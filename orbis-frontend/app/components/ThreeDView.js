        ...
        mtlLoader.load(assetTreeMtl.localUri, (materials) => {
          ...
          objLoader.load(assetTreeObj.localUri, (tree) => {
            ...
            mtlLoader.load(assetCloudMtl.localUri, (cloudMaterials) => {
              ...
              cloudObjLoader.load(assetCloudObj.localUri, (cloud) => {
                ...
                for (let i = 0; i < cloudNumber; i++) {
                  ...
                  const scale =
                    Math.random() * (maxCloudSize - minCloudSize) +
                    minCloudSize;
            ...
            for (let i = 0; i < treeNumber; i++) {
              ...
              const scale =
                Math.random() * (maxTreeSize - minTreeSize) + minTreeSize;
              dummy.scale.set(scale, scale, scale);
        ...
        if (startDistance.current && distance.current) {
          const futureZoom =
            ...
            (startDistance.current - distance.current) *
              zoomSensitivity.current;
          if (
            futureZoom > zoomLimitIn.current &&
            futureZoom < zoomLimitOut.current
          ) {
